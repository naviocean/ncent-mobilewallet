export const EMAIL_CHANGED = 'email_changed';
export const PASSWORD_CHANGED = 'password_changed';
export const LOGIN_USER_SUCCESS = 'login_user_success';
export const LOGIN_USER_FAIL = 'login_user_fail';
export const LOGIN_USER = 'login_user';

export const USER_UPDATE = 'user_update';
export const CREATE_USER = 'create_user';
export const CREATE_USER_SUCCESS = 'create_user_success';
export const CREATE_USER_FAIL = 'create_user_fail';
export const TEMP_PIN_ENTERED = 'temp_pin_entered';
export const PINS_DONT_MATCH = 'pins_dont_match';
export const PINS_DO_MATCH = 'pins_do_match';
export const EMAIL_ALREADY_EXISTS = 'email_already_exists';
export const EMAIL_CREATION_SUCCESS = 'email_creation_success';
export const EMAIL_VERIFICATION_SUCCESS = 'email_verification_success';
export const EMAIL_VERIFICATION_FAIL = 'email_verification_fail';

export const SEND_INFO_UPDATE = 'send_info_update';
export const SEND_TOKENS = 'send_tokens';
export const SEND_TOKENS_SUCCESS = 'send_tokens_success';
export const SEND_TOKENS_FAIL = 'send_tokens_fail';
export const CLEAR_TRANSACTION_INPUTS = 'clear_transaction_inputs';

export const GET_BALANCE = 'get_balance';
export const GET_BALANCE_SUCCESS = 'get_balance_success';
export const GET_BALANCE_FAIL = 'get_balance_fail';
export const CLEAR_BALANCE_DETAILS = 'clear_balance_details';

export const GET_ALL_TOKENS = 'get_all_tokens';
export const GET_TOKENS_SUCCESS = 'get_tokens_success';
export const GET_TOKENS_FAIL = 'get_tokens_fail';

export const MAITRE_EMAIL_CHANGED = 'maitre_email_changed';
export const MAITRE_SIGNUP_SUCCESS = 'maitre_signup_success';
export const MAITRE_SIGNUP_FAIL = 'maitre_signup_fail';
export const MAITRE_SIGNUP_SUBMIT = 'maitre_signup_submit';

export const USER_LOGOUT = 'user_logout';

export const SHOW_PHRASE = 'show_phrase';
export const HIDE_PHRASE = 'hide_phrase';

export const USER_LOGOUT_FROM_SIDEMENU = 'user_logout_from_sidemenu';

export const PASSWORD_CHANGED_IN_MODAL = 'password_changed_in_modal';
export const LOGIN_USER_SUCCESS_IN_MODAL = 'login_user_success_in_modal';
export const LOGIN_USER_FAIL_IN_MODAL = 'login_user_fail_in_modal';
export const LOGIN_USER_IN_MODAL = 'login_user_in_modal';

export const INVITATION_CODE_CHANGED = 'invitation_code_changed';
export const ALLOCATION_UPDATE = 'allocation_update';
export const INVESTOR_INFO_UPDATE = 'investor_info_update';